#!/usr/bin/env python3
"""
HeteroConv Model Evaluation Script

Evaluates the HeteroConv model (best_model_heteroconv.pt) on the validation set.
...
Output: data/processed/figures/heteroconv_*.png
        Console metrics
"""
...
def plot_roc_curve(preds, labels, output_dir, model_name="heteroconv"):
    """Plot ROC curve."""
    ...
    plt.title('ROC Curve - HeteroConv Model', fontsize=14, fontweight='bold')
...
def plot_precision_recall_curve(preds, labels, output_dir, model_name="heteroconv"):
    ...
    plt.title('Precision-Recall Curve - HeteroConv Model', fontsize=14, fontweight='bold')
...
def plot_confusion_matrix(metrics, output_dir, threshold, model_name="heteroconv"):
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate HeteroConv Model")
    ...
    print("=" * 60)
    print("  HeteroConv Model Evaluation")
    print("=" * 60)
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        print("Please train the HeteroConv model first.")
        sys.exit(1)
    ...
    print(f"\n[3/4] Loading and evaluating HeteroConv model...")
    checkpoint = torch.load(MODEL_PATH, map_location=device, weights_only=False)
    
    from train_heteroconv import HeteroConvFraudDetector
    ...
    plot_roc_curve(preds, val_labels, OUTPUT_DIR, "heteroconv")
    plot_precision_recall_curve(preds, val_labels, OUTPUT_DIR, "heteroconv")
    plot_confusion_matrix(metrics, OUTPUT_DIR, args.threshold, "heteroconv")
