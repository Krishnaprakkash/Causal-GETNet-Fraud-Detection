...
class FraudDetector(nn.Module):
    """
    Heterogeneous GNN for fraud detection using TransformerConv.
    
    Architecture:
    - Entity embeddings: 11 types × nn.Embedding(10000, hidden_dim)
    - TransformerConv layers for heterogeneous message passing
...
class SimplifiedFraudDetector(nn.Module):
    """
    Simplified heterogeneous GNN for fraud detection.
    
    Uses HeteroLinear for entity projection and mean aggregation.
    """
    ...
    def forward(self, data):
        """
        Forward pass with mean aggregation over entity neighbors.
        """
        ...
        for entity_type in self.entity_types:
            ...
            if not has_edges:
                entity_messages_list.append(
                    torch.zeros(tx_x.size(0), self.hidden_dim, device=tx_x.device)
                )
                continue
            ...
            num_tx = tx_x.size(0)
            agg_h = torch.zeros(num_tx, self.hidden_dim, device=tx_x.device)
            ...
            # Sum messages
            agg_h.index_add_(0, dst, entity_emb[src])
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PJT2 Baseline Model Training")
    ...
    # Create model
    print(f"\n[3/5] Creating FraudDetector model...")
    model = SimplifiedFraudDetector(
    ...
    print(f"      Entity bucket size: {NUM_ENTITY_NODES} (reduced from 10000)")
